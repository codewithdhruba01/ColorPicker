...
import { Search, Copy, Check, Heart, ArrowLeft } from "lucide-react"
...
export default function AllGradientsPage() {
  ...
  const toggleLike = (gradientId: string) => {
    setLikedGradients(prev => {
      ...
      if (newSet.has(gradientId)) {
        newSet.delete(gradientId)
      } else {
        newSet.add(gradientId)
      }
      return newSet
    })
  }
                  ...
                  >
                    ...
                    >
                        ...
                        <div className="transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                          {copiedId === gradient.id ? (
                            ...
                            <div className="flex flex-col items-center gap-3">
                              ...
                              <p className="text-white text-sm font-medium">Click to Copy CSS</p>
                              <div className="px-4 py-2 bg-black/40 backdrop-blur-sm rounded-lg border border-white/20 max-w-[90%]">
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm border border-white/30 flex items-center justify-center hover:bg-white/30 transition-all z-10"
                      >
                        <Heart
                          className={`w-5 h-5 ${
